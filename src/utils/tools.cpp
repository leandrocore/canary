...
std::string generateToken(const std::string &key, uint32_t ticks) {
	...
	for (uint8_t i = 0; i < key.length(); ++i) {
		iKeyPad[i] = safe_convert<char>(safe_convert<unsigned char>(iKeyPad[i]) ^ safe_convert<unsigned char>(key[i]), __FUNCTION__);
		oKeyPad[i] = safe_convert<char>(safe_convert<unsigned char>(oKeyPad[i]) ^ safe_convert<unsigned char>(key[i]), __FUNCTION__);
	}
