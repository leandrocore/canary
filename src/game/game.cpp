...
uint32_t Game::makeInfluencedMonster() {
	...
	while (true) {
		...
		auto random = safe_convert<uint32_t>(normal_random(0, safe_convert<int32_t>(forgeableMonsters.size() - 1)), __FUNCTION__);
...
uint32_t Game::makeFiendishMonster(uint32_t forgeableMonsterId /* = 0*/, bool createForgeableMonsters /* = false*/) {
	...
	while (true) {
		...
		auto random = safe_convert<uint32_t>(uniform_random(0, safe_convert<int32_t>(forgeableMonsters.size() - 1)), __FUNCTION__);
