	...
	void calculateAbsorbValues(std::shared_ptr<Player> player, NetworkMessage &msg, uint8_t &combats) {
		...
		for (size_t i = 0; i < COMBAT_COUNT; ++i) {
			...
			if (g_configManager().getBoolean(TOGGLE_WHEELSYSTEM, __FUNCTION__)) {
				damageReduction[i] -= safe_convert<uint16_t>(player->wheel()->getResistance(indexToCombatType(i)) / safe_convert<int32_t>(100.f), __FUNCTION__);
			}
			...
			if (damageReduction[i] != 100) {
				...
				msg.addByte(std::max<int8_t>(-100, std::min<int8_t>(100, safe_convert<int8_t>(std::max(0, 100 - safe_convert<int>(damageReduction[i])), __FUNCTION__))));
				++combats;
			}
		}
	}
...
void ProtocolGame::sendDistanceShoot(const Position &from, const Position &to, uint16_t type) {
	...
	if (oldProtocol) {
		...
		msg.addByte(safe_convert<uint8_t>(safe_convert<int8_t>(safe_convert<int32_t>(to.x) - safe_convert<int32_t>(from.x)), __FUNCTION__));
		msg.addByte(safe_convert<uint8_t>(safe_convert<int8_t>(safe_convert<int32_t>(to.y) - safe_convert<int32_t>(from.y)), __FUNCTION__));
...
void ProtocolGame::sendCreatureHealth(std::shared_ptr<Creature> creature) {
	...
	if (creature->isHealthHidden()) {
		...
		msg.addByte(safe_convert<uint8_t>(std::min<double>(100, std::ceil((safe_convert<double>(creature->getHealth()) / std::max<int32_t>(creature->getMaxHealth(), 1)) * 100)), __FUNCTION__));
	}
