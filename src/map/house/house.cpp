...
uint32_t House::getRent() const {
	return safe_convert<uint32_t>(g_configManager().getFloat(HOUSE_RENT_RATE, __FUNCTION__) * safe_convert<float>(rent), __FUNCTION__);
}
...
uint32_t House::getPrice() const {
	...
	auto rentPrice = safe_convert<uint32_t>(safe_convert<float>(getRent()) * g_configManager().getFloat(HOUSE_PRICE_RENT_MULTIPLIER, __FUNCTION__), __FUNCTION__);
