...
Spectators Spectators::find(const Position &centerPos, bool multifloor, bool onlyPlayers, int32_t minRangeX, int32_t maxRangeX, int32_t minRangeY, int32_t maxRangeY) {
	...
	if (multifloor) {
		if (centerPos.z > MAP_INIT_SURFACE_LAYER) {
			minRangeZ = safe_convert<uint8_t>(std::max(safe_convert<int>(safe_convert<int8_t>(centerPos.z)) - MAP_LAYER_VIEW_LIMIT, 0), __FUNCTION__);
			maxRangeZ = safe_convert<uint8_t>(std::min(safe_convert<int>(safe_convert<int8_t>(centerPos.z)) + MAP_LAYER_VIEW_LIMIT, safe_convert<int>(MAP_MAX_LAYERS - 1)), __FUNCTION__);
