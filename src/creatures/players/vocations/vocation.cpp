...
uint64_t Vocation::getReqSkillTries(uint8_t skill, uint16_t level) {
	...
	uint64_t tries = safe_convert<uint64_t>(skillBase[skill] * std::pow(safe_convert<double>(skillMultipliers[skill]), level - (minSkillLevel + 1)), __FUNCTION__);
...
uint64_t Vocation::getReqMana(uint32_t magLevel) {
	...
	uint64_t reqMana = safe_convert<uint64_t>(std::floor(1600.0 * std::pow(manaMultiplier, safe_convert<double>(magLevel) - 1)), __FUNCTION__);
