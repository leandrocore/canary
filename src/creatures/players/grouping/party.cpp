...
uint32_t Party::getMinLevel() {
	return safe_convert<uint32_t>(std::ceil((safe_convert<float>(getHighestLevel()) * 2) / 3), __FUNCTION__);
}
...
uint32_t Party::getMaxLevel() {
	return safe_convert<uint32_t>(std::floor((safe_convert<float>(getLowestLevel()) * 3) / 2), __FUNCTION__);
}
