...
void Player::parseAttackRecvHazardSystem(CombatDamage &damage, std::shared_ptr<Monster> monster) {
	...
	if (monster->getHazardSystemCrit() && (lastHazardSystemCriticalHit + g_configManager().getNumber(HAZARD_CRITICAL_INTERVAL, __FUNCTION__)) <= OTSYS_TIME() && chance <= critChance && !damage.critical) {
		...
		damage.primary.value += safe_convert<int32_t>(std::ceil((safe_convert<double>(damage.primary.value) * (5000 + stage)) / 10000), __FUNCTION__);
		damage.secondary.value += safe_convert<int32_t>(std::ceil((safe_convert<double>(damage.secondary.value) * (5000 + stage)) / 10000), __FUNCTION__);
	...
	if (monster->getHazardSystemDamageBoost() /* && !damage.critical*/) {
		...
		if (stage != 0) {
			damage.extension = true;
			damage.exString = "(Hazard)";
			damage.primary.value += safe_convert<int32_t>(std::ceil((safe_convert<double>(damage.primary.value) * stage) / 10000), __FUNCTION__);
			if (damage.secondary.value != 0) {
				damage.secondary.value += safe_convert<int32_t>(std::ceil((safe_convert<double>(damage.secondary.value) * stage) / 10000), __FUNCTION__);
			}
		}
	}
}
