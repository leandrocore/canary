...
void Npc::onPlayerBuyItem(std::shared_ptr<Player> player, uint16_t itemId, uint8_t subType, uint16_t amount, bool ignore, bool inBackpacks) {
	...
	if (inBackpacks && itemType.stackable) {
		bagsCost = shoppingBagPrice * safe_convert<uint32_t>(std::ceil(std::ceil(safe_convert<double>(amount) / itemType.stackSize) / shoppingBagSlots), __FUNCTION__);
	} else if (inBackpacks && !itemType.stackable) {
		bagsCost = shoppingBagPrice * safe_convert<uint32_t>(std::ceil(safe_convert<double>(amount) / shoppingBagSlots), __FUNCTION__);
	}
