	...
	if (it.abilities) {
		...
		for (int32_t s = STAT_FIRST; s <= STAT_LAST; ++s) {
			...
			if (it.abilities->statsPercent[s]) {
				player->setVarStats(safe_convert<stats_t>(s, __FUNCTION__), safe_convert<int32_t>(player->getDefaultStats(safe_convert<stats_t>(s)) * ((it.abilities->statsPercent[s] - 100) / 100.f), __FUNCTION__));
			}
		}
	}
